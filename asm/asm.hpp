...
struct Compiler {
  ...
  void UpdateCalles(const char* name) {
    auto fd = label.find(name);
    u64 cur = fd->second;
    mew::stack<u64> deletes4;
    for (auto [k, v] : calles) {
      if (!mew::strcmp(v, name)) continue;
      cb.putAtCode(k, cur);
      deletes4.push(k);
    }
    for (u64 i = 0; i < deletes4.count(); ++i) { 
      calles.erase(deletes4[i]);
    }
    if (calles.size() != 0) {
      for (auto [k, v] : calles) {
        MewWarn("Not Found label (%s)", v);
      }

      MewCritical("^^^^^ NOT DEFINED LABELS ^^^^^");
      exit(1);
    }
  }
  ...
  void PutArg(Token& tk) {
    ...
    switch (tk.type) {
      ...
      default:
        MewUserAssert(false, "NOT FOR VM_ARG");
    }
  }
